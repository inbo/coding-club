...
# 1. How to add province, WBE name, and surface area to each observation (where available)?
left_join(partridge, WBE, by = "wbe")

# or using pipe `%>%`
partridge %>%
  left_join(WBE, by = "wbe")

# you can omit `by = "wbe"` in this case as all join functions automatically
