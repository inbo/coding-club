# Plotting, modelling and mapping

This section up to the end of the book is derived from the second challenge.
...
We fold long distracting code chunks by adding the chunk option `class.source = "fold-hide"`. It's up to the reader to unfold the code if interested.

### Catches per province

```{r catch-per-province, class.source = "fold-hide", fig.cap = "Total number of catched geese per province from 2009 to 2018"}
...
```{r catch-per-year, class.source = "fold-hide", fig.cap = "Annual number of catched geese"}
...
```{r catch-year-pr, class.source = "fold-hide", fig.cap = "Catched geese per year from 2009 to 2018. The color indicates the province where catch occurred."}
...
```{r species-to-model, class.source = "fold-hide"}
species <- c(
  "Brandgans",
  "Canadese gans",
  "Grauwe gans",
...
```{r catch-species, class.source = "fold-hide", message=FALSE}
catch_species <-
  catch_fl %>%
...
```{r apply-gee-model, class.source = "fold-hide"}
model_per_species <-
  map(
    species,
    function(s) {
...
```{r show_model_plots, class.source = "fold-hide", fig.cap = "Modelized catch data for five species from 2010 to 2018."}
ggplot(overview_gee,
...
```{r leaflet-maps, class.source = "fold-hide", fig.cap="Map showing the total number of catches per province."}
