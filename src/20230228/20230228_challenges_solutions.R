...
# output of unique() is a vector. unique() is a basic R function
unique(partridge$type)
length(unique(partridge$type))
# output of dplyr function distinct() is a data.frame with one column
...
# Add a new column called `seen` which is TRUE if `type` is not `"Sound"`. Add a column `month` with the month of the observation. Tip: use `lubridate::month()` function
partridge <- mutate(partridge,
                    seen = if_else(type != "Sound", TRUE, FALSE),
                    month = (lubridate::month(date))
)

# Why does a dplyr if_else() function exists if basic R provides already
# `ifelse()`? Check if_else() documentation to get the answer:
# https://dplyr.tidyverse.org/reference/if_else.html
# Short answer: better handling missing values and preserve type
x <- c(-5:5, NA)
...
# What if I drop the groups before filtering?
partridge %>%
  group_by(month, wbe) %>%
  summarise(n_obs = n(), .groups = "drop") %>%
...
WBE %>% right_join(overview_partridge, by = c("wbe" = "wbe"))
...
overview_partridge %>% inner_join(WBE, by = c("wbe" = "wbe"))


# 5. Now, join both again but retain ALL EXISTING WBEs. Set 0 to WBEs where no partridges were observed.
WBE %>% left_join(overview_partridge, by = c("wbe" = "wbe")) %>%
  mutate(n_obs = replace_na(n_obs, 0))

## BONUS CHALLENGE 1 - Tidy data

# After reading `20230228_ias_plants.tsv` dataset (code provided), make it tidy. Please, notice that for some species two vernacular names are given. See figure below.
ias <- read_csv("./data/20230228/20230228_ias_plants.tsv", na = "")
head(ias)
ias <- ias %>%
  pivot_longer(
