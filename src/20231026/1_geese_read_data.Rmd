...
Set *Global Options* -\> *RMarkdown pane* -\> *Evaluate chunks in directory*: **Project**. In this way RMarkdown knows that he has to read paths starting from the root folder, i.e. the folder containing `coding-club.Rproj`.
...
```{r read-again-with-here}
library(here) # install it first if not yet done
path_catch_file <- here("data", "20231026", "20231026_geese_counts.txt")
...
catch_fl %>% dplyr::filter(is.na(province) | is.na(location)) %>% nrow
```
...
write_csv(
  ...
  file = here("data",
              "20231026",
              "20231026_geese_counts_cleaned.txt"),
  na = ""
)
```
