...
#' function solutions are saved in `20220630_functions_solutions.R`
source("./src/20220630/20220630_functions_solutions.R")
...
#' folder to allow using some different directtories than "./data/20220630/"
ha_2011 <- get_obs_extra(species = "Harmonia axyridis",
                   year = 2011,
                   root_folder = "./data/20220630/")
...
cells_with_obs_ha_2011 <- add_n_obs_to_eea_grid(
  n_obs_per_cell = n_obs_cell_ha_2011,
  grid_cells = be_grid
)
cells_with_obs_cb_2010 <- add_n_obs_to_eea_grid(
...
# Before showing how to create a macro function, take a look on how you can
# "pipe" the previous functions to create a readable workflow.
# Notice this is possible only when the output of a function is used as first
# argument of the next function, which is our case.

# read input data
ha_2010 <- get_obs(species = "Harmonia axyridis", year = 2010)
eea_be_grid <- read_sf("./data/20220630/20220630_eea_1x1km_grid_BE.gpkg")

ha_2010 %>%
  clean_data() %>%
  assign_eea_cell(longitude_colname = "decimalLongitude",
                  latitude_colname = "decimalLatitude") %>%
  get_n_obs_per_cell() %>%
  add_n_obs_to_eea_grid(be_grid = eea_be_grid) %>%
  visualize_obs_cells()
...
#' example to return NULL and a warning message if no rows are left.
#' Improved function is called `make_report_improved()`
...
# Read EEA grid
be_grid <- st_read("./data/20220630/eea_1x1km_grid_BE.geojson")
