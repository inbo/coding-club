...
taxon %>% head()
taxon %>% colnames()
taxon %>% select(NaamNederlands) %>% head(n = 10)
taxon %>% select(NaamNederlands) %>% head(n = 10) %>% collect()
taxon %>%
...
userflora_dutch_to_scientificname_extended <- function(table, dutchname) {
  table %>%
    # YOUR CODE ...
    filter(tolower(NaamNederlands) %LIKE% paste0("%", dutchname, "%")) %>%
    select(NaamWetenschappelijk)
  #
}
...
taxon %>%
  ...
  select(-c(TaxonGroepID, Omschrijving)) %>%
  # to check
  head(100) %>%
  collect() %>%
  View()
